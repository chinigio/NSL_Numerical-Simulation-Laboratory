  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        coutpv << "#     BLOCK:   VELOCITY:   ACT_POFV:    AVE_POFV:        ERROR:" << endl;
        coutpv.close();
        input>>_n_bins_v;
        _nprop += _n_bins_v;
...
void System :: averages(int blk){
  ...
  if (_measure_pofv){
    ...
    for(int i=0; i<_n_bins_v; i++){
      double velocity = _bin_size_v * (i + 0.5);
      double norm = 1.0 / ((double)_npart * _bin_size_v); // normalization factor
      average  = _average(_index_pofv+i) * norm;
      sum_average = _global_av(_index_pofv+i) * norm;
      sum_ave2 = _global_av2(_index_pofv+i) * pow(norm,2);
      coutf << setw(12) << blk
            << setw(12) << velocity
            << setw(12) << average
            << setw(12) << sum_average/double(blk)
